...
author: CEVE 543 Fall 2025 lab 2
date: "2025-09-05"
type: "lab"
module: 1
week: 2
...
function plot_stations()
	...
	scatter!(ga, stations.longitude, stations.latitude;
		color = stations.years_of_data, colormap = :plasma, markersize = 10)  # <3>
	...
	# Add colorbar
	Colorbar(fig[1, 2], label = "Years of Data", colormap = :plasma,
...
3. Plot weather stations colored by years of data using the plasma color scheme
...
```{julia}
target_location = "UT Austin"  # <1>
target_lon = -97.73942              # <2>
target_lat = 30.28614               # <3>
```
...
1. Generate evenly-spaced colors from the plasma palette for each station
...
μ = 2.4   # Location parameter - shifts the distribution left/right
σ = 1.8    # Scale parameter - controls spread (must be > 0)
...
Ans: I tried to plot data near UT Austin. From my visual observation, 63-0192(Barton CRK at Loop 360) tends to have a positive trend. As from timeline of 2000-2023, the annual max rainfall tends to increase rather than before 2000.
...
Ans:  From my visual observation, 41-0420(Austin) station tends to have negative trend, although this station doesn't completely face a negative trend every year, it has also some spike increase in certain year. But, long term observation seems like the data is facing a negative trend.
...
- Final: $\mu = 2.4$ $\sigma = 1.8$, $\xi = 0.05$
...
Ans: I noticed when I increased the location parameter to 6.0, the value of return level range shifts in betw 5 and 16 RL in y axis.

When the location parameter is 4, the y axis range is 4 to 15.

When I changed it to 2.0, the range is approx 1.5 to 12.5 in y axis.

So, that means, with increase in location parameter, the curve shifts upward, and decrease in location parameter shifts curve downward, but the range remains same.

**What happened when you increased/decreased $\sigma$ (scale parameter)?**

*Test values like $\sigma = 0.3, 1.3, 2.0$. Describe how the curve became steeper/flatter and how this affected the spread of return levels.*
Ans: When I changed sigma, say 2.0, the spread of values in y axis change,for 2.0, the value of return level ranges from 4 to 21 inch approx.

With value of 1.3, the range is 3.5 to 15 inch approx.

With value of 0.3, the range is 3.5 to 6.5 inch approx.

That means, scale parameter actually determines the spread of the curve, with increased sigma, the range in y axis(Return level) increases and curve becomes more steeper.

We can consider. it as y=mx+c equation for the inverse CDF, where m is equivalent to sigma(scale parameter), and c(location parameter) is equavelant to mu. So, sigma increases mean m(slope in straight line eqn) is increasing, and that means the y axis value range(y2-y1) (RL) increases. (y2-y1) is the numerator in slope eqn, so it should increase for the slope value to be increased.

On the other hand, mu or c increases mean, the intercept(staright line eqn) is increasing, which results shift of the curve in upward direction.
...
Ans: with negative shape parameter, the curve flattens to its upper value, but with increase in shape parameter the curve becomes more steep and y axis value increases rapidly.

For -0.3, 0,0.15 and 0.3, I got RL value as 7.5,11,15 and 22
...
Ans: The graph shows that the red dot(weibull position) follows closely with the blue line(GEV). That means GEV is representing well to the closest station data, but the maximum deviation seems to happen to the increased return period where GEV underestimate the station data.

**Additional observations:**

*Optional: Add any other interesting patterns you notice in the return period analysis.*

Ans: I selected one location in El Paso, and tried to plot the nearest station data which is El Paso, I saw during different time period there are missing data in this station. So, from these kind of stations, rainfall trend prediction can be challenging. 
